...
internal class DocCropAreaView @JvmOverloads constructor(
    context: Context,
    ...
    private val validShapeLineColor = getColor(R.color.doc_crop_area_valid_shape_line_color)
    private val invalidShapeLineColor = getColor(R.color.doc_crop_area_invalid_shape_line_color)

    private val lineStrokeWidth = getDimension(R.dimen.doc_crop_area_line_stroke_width)
    ...
    private fun initImageView(context: Context): AppCompatImageView {
        return AppCompatImageView(context)
            .apply {
                ...
                setImageResource(R.drawable.image_crop_area_pointer)
            }
    }
    ...
    fun setCropArea(cropArea: DocCropArea) {
        topLeftPointerIv.x = cropArea.topLeftCoord.x
        topLeftPointerIv.y = cropArea.topLeftCoord.y

        topRightPointerIv.x = cropArea.topRightCoord.x
        topRightPointerIv.y = cropArea.topRightCoord.y

        bottomLeftPointerIv.x = cropArea.bottomLeftCoord.x
        bottomLeftPointerIv.y = cropArea.bottomLeftCoord.y

        bottomRightPointerIv.x = cropArea.bottomRightCoord.x
        bottomRightPointerIv.y = cropArea.bottomRightCoord.y
    }


    fun getCropArea(): DocCropArea? {
        val docShape = (getOrderedCropAreaCoords() ?: return null)
    ...
    private fun getOrderedCropAreaCoords(): DocShape? {
        ...
        return docCoordsOrderer.order(coords)
    }


    fun hasValidArea(): Boolean {
        return (getOrderedCropAreaCoords() != null)
    }
    ...
    private inner class MiddlePointerTouchListener(
        ...
        private fun View.isWithinHorizontalBounds(edgePointer: View, currentPoint: PointF): Boolean {
            return (
                ...
                ((edgePointer.x + currentPoint.x + width) < this@DocCropAreaView.width)
            )
        }
        ...
        private fun View.isWithinVerticalBounds(edgePointer: View, currentPoint: PointF): Boolean {
            return (
                ...
                ((edgePointer.y + currentPoint.y + height) < this@DocCropAreaView.height)
            )
        }
